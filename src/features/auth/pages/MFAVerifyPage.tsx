    ...
    mfaChallengeMutation.mutate(
      { user_id: userId, code: code.trim().toUpperCase() },
      {
        onSuccess: async (res) => {
          ...
          // ! set tokens so axios can attach the Bearer header for the profile fetch
          setAuth(
            {
              id: userId,
              email: "",
          ...
          // ! fetch full profile right away
          try {
            const profile = await authApi.getProfile();
            setAuth(profile.data, access_token, refresh_token);
          } catch {
            // non-fatal — useProfileBootstrap retries on next page
          }
          navigate("/");
        },
      }
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input
            ...
            style={{
              ...
              padding: "11px 14px",
              textAlign: "center",
              border: codeError ? "1px solid var(--error)" : "1px solid var(--outline-strong)",
              borderRadius: 10,
              background: "white",
              outline: "none",
            }}
            ...
            onBlur={(e) => {
              e.currentTarget.style.borderColor = codeError
                ? "var(--error)"
                : "var(--outline-strong)";
            }}
          />
        ...
        <SeuSubmitButton type="submit" loading={mfaChallengeMutation.isPending} className="mt-2">
          Verify
        </SeuSubmitButton>
      </form>
      <p
