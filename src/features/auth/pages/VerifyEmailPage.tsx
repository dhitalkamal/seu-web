...
const successStyle = { background: "#dcfce7", color: "#16a34a", border: "1px solid #bbf7d0", fontFamily: "Manrope, sans-serif" };
...
export default function VerifyEmailPage() {
  ...
  if (!email) {
    return (
      <AuthLayout eyebrow="Verification" title="Check your" titleAccent="email">
        <p style={{ fontSize: 13, color: "var(--on-var)", fontFamily: "Manrope, sans-serif", marginBottom: 16 }}>
          We sent a verification code to your inbox.
        </p>
        <Link to="/register" className="font-bold no-underline hover:underline" style={{ color: "var(--primary)", fontSize: 13, fontFamily: "Manrope, sans-serif" }}>
          Back to sign up
        </Link>
      </AuthLayout>
    );
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!validate()) return;
    verifyEmailMutation.mutate({ email, otp: otp.trim().toUpperCase() }, {
      onSuccess: () => navigate("/login", { state: { flash: "Email verified! You can now sign in." } }),
    });
  }

  return (
    <AuthLayout eyebrow="Almost there" title="Verify your" titleAccent="email" subtitle={`Enter the 8-character code sent to ${email}`}>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        {verifyEmailMutation.isError && <div className="px-4 py-3 rounded-xl text-sm" style={errorStyle}>{getApiError(verifyEmailMutation.error)}</div>}
        {resent && <div className="px-4 py-3 rounded-xl text-sm" style={successStyle}>A new code has been sent to {email}.</div>}
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input
            type="text"
            value={otp}
            onChange={(e) => { setOtp(e.target.value.toUpperCase()); setResent(false); }}
        ...
        <SeuSubmitButton type="submit" loading={verifyEmailMutation.isPending} className="mt-2">Verify email</SeuSubmitButton>
      </form>
      ...
      <div className="flex flex-col items-center gap-2 mt-6" style={{ fontSize: 13, fontFamily: "Manrope, sans-serif" }}>
        <p style={{ color: "var(--on-var)" }}>
          ...
          <button type="button" onClick={() => { resendOTPMutation.mutate(email, { onSuccess: () => setResent(true) }); }} disabled={resendOTPMutation.isPending}
            style={{ color: "var(--primary)", fontWeight: 700, background: "none", border: "none", cursor: "pointer", opacity: resendOTPMutation.isPending ? 0.5 : 1 }}>
